...
> ·對於MiHoYo/HoYoVerse開發商在公測後面向測試者的Beta版客戶端(通常客戶端版本號爲:現行版本號.5.X)，HoYoShade僅對被泄露出來的客戶端進行測試並支持，不代表所有版本的內測客戶端均可運行HoYoShade。如果你正在使用的測試服客戶端並不支持運行HoYoShade，並且你非常確定這並非是使用了錯誤的注入選項導致的問題，請在[issues界面](https://github.com/DuolaD/HoYoShade/issues)新建issue以幫助我們適配。或者使用啓動器的開發者功能嘗試自定義注入。  
...
| **絕區零/ZZZ/Zenless Zone Zero(中國大陸/海外 1.1版本內測)(1.1.X)*** | 是 | 是 | 無遊戲版本限制 |

*:已開發完畢，隨ReShade版本更新推出(該注入方案可用於後續所有內測版本客戶端)

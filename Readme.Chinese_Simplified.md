...
> [!NOTE]
> ·我们深知您可能熟悉第二语言。但如果你来自于中国大陆地区,我们更加推荐你去阅读[简体中文版本的Readme.md](Readme.Chinese_Simplified.md)来获得更多有关于原神无人机的信息。  
...
> ·我们不建议你使用Beta客户端在官方服务器中运行HoYoShade，以免对HoYoShade全体开发者/全体二创开发者和你本人造成保密协议违约/违规/违法/经济损失等风险。  
...
> ·对于MiHoYo/HoYoVerse开发商在公测后面向测试者的Beta版客户端(通常客户端版本号为:现行版本号.5.X)，HoYoShade理论上也可以提供注入和运行支持。但我们不建议你使用Beta客户端在官方服务器中运行HoYoShade，以免对HoYoShade全体开发者/二创开发者和你本人造成保密协议违约/违规/违法/经济损失等风险。  
> ·对于MiHoYo/HoYoVerse开发商在公测后面向测试者的Beta版客户端(通常客户端版本号为:现行版本号.5.X)，HoYoShade仅对被泄露出来的客户端进行测试并支持，不代表所有版本的内测客户端均可运行HoYoShade。如果你正在使用的测试服客户端并不支持运行HoYoShade，并且你非常确定这并非是使用了错误的注入选项导致的问题，请在[issues界面](https://github.com/DuolaD/HoYoShade/issues)新建issue以帮助我们适配。或者使用启动器的开发者功能尝试自定义注入。  
> ·对于并未出现在支持游戏列表中的游戏，你可以自行尝试选择使用注入器中已支持的游戏/在开发者选项中选择自定义注入来尝试注入HoYoShade。  
> ·HoYoShade支持同时与其它主流Mod一并注入，且不存在任何冲突问题。但请注意并确保其它游戏Mod之间不会有冲突（例：原神无人机与GIMI和甜瓜）。  
...
| **绝区零/ZZZ/Zenless Zone Zero 中国大陆 1.0 公测客户端** | 是 | 是 | - |
| **绝区零/ZZZ/Zenless Zone Zero 海外 1.0 公测客户端** | 是 | 是 | - |
| **绝区零/ZZZ/Zenless Zone Zero(中国大陆/海外 1.1版本内测)(1.1.X)*** | 是 | 是 | 无游戏版本限制 |

*:已开发完毕，随ReShade版本更新推出(该注入方案可用于后续所有内测版本客户端)
...
## 〢 截图展示
<div align="center">
  <h1 class="header">截图来自[阿向菌|AXBro]</h1>
</div>
...
<div align="center">
    <table>
        ...
        <tr>
            ...
            <td>
		<a>由于我们的贡献者'阿向菌|AXBro'决定出售自己公开的社交媒体账号</a><br>
		<a>我们已将其联系方式从本文档中移除，以避免账号交易后产生不必要的误会。</a>
            </td>
        </tr>
        <tr>
            <td>
